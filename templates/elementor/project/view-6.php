...
?>
<div class="rt-project-default rt-project-multi-layout-6 project-grid-<?php echo esc_attr( $layout );?>">
	<div class="row <?php echo esc_attr( $item_space );?>">
		<?php $count = 1; $ade = $delay; $adu = $duration; if ( $query->have_posts() ) {
			while ( $query->have_posts() ) {
				...
				?>
                <div class="col-lg-<?php echo esc_attr($cols) ?> <?php echo esc_attr( $animation );?> <?php echo esc_attr( $animation_effect );?>" data-wow-delay="<?php echo esc_attr( $ade );?>ms" data-wow-duration="<?php echo esc_attr( $adu );?>ms">
		                ...
		                <?php if( $button_display ) { ?>
                            <div class="rt-button">
	                            <?php if( $link_popup == 'popup' ) { ?>
                                    <a class="btn button-4" href="<?php echo esc_url( wp_get_attachment_url( get_post_thumbnail_id() ) ); ?>"
	                            ...
	                            <?php } else { ?>
                                    <a class="btn button-4" href="<?php the_permalink();?>" aria-label="project link"><?php Icons_Manager::render_icon( $button_icon ); ?></a>
	                            <?php } ?>
                            </div>
		                <?php } ?>
                    </div>
