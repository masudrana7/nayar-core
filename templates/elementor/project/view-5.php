...
if( $project_thumbnail_size ) {
	...
	$thumb_size = 'nayar-size2';
}

$query = new WP_Query( $args );

?>
<div class="rt-project-default rt-project-multi-layout-5 project-grid-<?php echo esc_attr( $layout );?>">
	<div class="item-gallery">
		<?php $count = 1; $ade = $delay; $adu = $duration; if ( $query->have_posts() ) {
			while ( $query->have_posts() ) {
				...
				?>
                <div class="item-grid-item grid-item-<?php echo esc_attr($count) ?> <?php echo esc_attr( $animation );?> <?php echo esc_attr( $animation_effect );?>" data-wow-delay="<?php echo esc_attr( $ade );?>ms" data-wow-duration="<?php echo esc_attr( $adu );?>ms">
                    <div class="project-item">
                        <div class="project-thumbs <?php echo esc_attr( $grayscale_display );?>">
	                        <?php nayar_post_thumbnail( esc_attr( $thumb_size ) ); ?>
                        </div>
                        <div class="project-content">
                            <div class="project-info">
				                ...
				                <?php } if ( $content_display == 'yes' ) { ?>
                                    <div class="project-excerpt"><?php nayar_html( $content , false ); ?></div>
                                <?php } if( $button_display ) { ?>
                                    <div class="rt-button">
	                                    <?php if( $link_popup == 'popup' ) { ?>
                                            <a class="btn button-1" href="<?php echo esc_url( wp_get_attachment_url( get_post_thumbnail_id() ) ); ?>"
	                                    ...
	                                    <?php } else { ?>
                                            <a class="btn button-1" href="<?php the_permalink();?>" aria-label="project link"><?php Icons_Manager::render_icon( $button_icon ); ?></a>
	                                    <?php } ?>
                                    </div>
                                <?php } ?>
                            </div>
            ...
            <?php $count++; $ade = $ade + 200; $adu = $adu + 0; } ?>
		<?php } ?>
	</div>
</div>