...
namespace RT\NAYAR\DemoImporter;

use RT\NAYAR\DemoImporter\Utils;
use RT\NAYAR\DemoImporter\Handlers\WooHandler;
...
class Core {
    ...
    public function radius_booking() {
        if ( ! class_exists( '\RadiusTheme\RadiusBooking\Facades\Import' ) ) {
            return $this;
        }
        $json_file_path = RDTHEME_CORE_DEMO_CONTENT . 'demo-files/radius-booking.json';
        if ( file_exists( $json_file_path ) ) {
            \RadiusTheme\RadiusBooking\Facades\Import::fromFile($json_file_path)->import();
        }
        return $this;
    }
